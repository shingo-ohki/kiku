...
// ユーザープロンプトの作成
function buildUserPrompt(req: GenerateRequest, mode: "default" | "lowered_entry"): string {
...
// JSON の検証と型変換
function parseGeneratedStructure(content: string) {
  // JSON ブロックを抽出
  ...
  try {
    ...
    if (!body.theme?.trim() || !body.background?.trim()) {
      return NextResponse.json(
        { error: "theme と background は必須です" },
        { status: 400 }
      )
    }
